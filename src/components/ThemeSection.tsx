  ...
  return (
    <section
      className={cn(
        ...
        hasBackground ? "bg-secondary" : "bg-background"
      )}
      aria-labelledby={`theme-${theme.slug}`}
    >
      {/* Decorative accent */}
      {hasBackground && (
        <div className="pointer-events-none absolute right-0 top-0 h-64 w-64 rounded-full bg-gold/[0.04] blur-3xl" />
      )}
      ...
      <div className="container relative">
        ...
        <div
          ...
          className={cn(
            "grid items-center gap-10 md:grid-cols-10",
        ...
        >
          ...
          >
            ...
            <Link
              to={`/${theme.slug}`}
              className="gold-underline mt-5 inline-flex items-center gap-2 text-sm font-semibold text-accent transition-colors hover:text-accent/80"
            >
        ...
        <div
          ref={cardsReveal.ref}
          className={cn(
            "mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 reveal-stagger",
